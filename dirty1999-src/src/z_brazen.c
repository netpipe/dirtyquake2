...
void BraZenIniInit(void)
{
        ...
        // Dirty
                ...
                while (i < 8)
                {
                        ...
                        strcpy(current_team, va("[team%i]", i));

                        //gi.dprintf("TeamInit - Team Section = %s\n", current_team);
                        do
                        {
                                fscanf(ini_file, "%s", line);
                        } while (!feof(ini_file) && (Q_stricmp(line, current_team) != 0));
                        ...
                        {       // Found Team heading.
                                ...
                                while ((!feof(ini_file)) && (n < 4))
                                {
                                        ...
                                        if (n == 1)
                                        {
                                                strcpy(team->name, line);
                                                //gi.dprintf("TeamInit - team->name = %s\n", line);
                                        }
                                        else if (n == 2)
                                        {
                                                strcpy(team->model, line);
                                                //gi.dprintf("TeamInit - team->model = %s\n", line);
                                        }
                                        else if (n == 3)
                                        {
                                                strcpy(team->skin, line);
